...
deliberately not in this path. That backward is the torch-composed
reverse-chunk loop `_delta_backward_torch` -- the verified, exact-to-eager
hand-derived recurrence (per-chunk transpose solve `G = T^-T dU`, reverse
`dH` carry) -- called DIRECTLY by `_DeltaScanFn.backward` as the one and
only backward.

NOTE (do not rebuild blind): a fully fused Triton backward trio MIRRORING
the forward decomposition was built and validated to full parity
(chunk-parallel transpose-solve hoist, serial reverse-`dH` pass,
chunk-parallel grad pass, SMEM-tuned to fit the whole envelope -- 15/15
classes engaged), but measured 8-12x SLOWER than `torch.compile` on an L4
(sm_89): fitting the ~101 KB opt-in shared-memory budget forced the
`tl.dot`s onto shapes that destroy tensor-core efficiency at this envelope,
a structural conflict rather than a tuning miss. Per the user's second
stop-rule decision (2026-07-19) it was REVERTED to this torch-composed loop
and its five kernels deleted; git history preserves them, so nobody should
rebuild it blind. Evidence: `experiments/bench/gpu_delta_probe.md` and the
intent ledger's 2026-07-19 addenda
(`.claude/output/intent/2026-07-18-delta-triton-kernel-intent.md`). Every
forward tile is padded to
`max(16, next_pow2(...))`
...
if _HAS_TRITON:
    ...
    class _DeltaScanFn(torch.autograd.Function):
        ...
        eager path's. The backward is the torch-composed reverse-chunk loop
        ``_delta_backward_torch``, called directly (no dispatcher, no
        fallback). A fully fused Triton backward trio was built and validated
        to full parity but measured 8-12x slower than ``torch.compile`` on an
        L4 (SMEM-fit vs tensor-core dot efficiency conflict) and was reverted
        per the user's 2026-07-19 stop-rule decision -- see the module
        docstring's "do not rebuild blind" note and
        ``experiments/bench/gpu_delta_probe.md``. ``@once_differentiable``
        ...
        def backward(ctx, dy, dH_T):
            """Torch-composed reverse-chunk backward (the one and only backward).

            Runs ``_delta_backward_torch`` directly, seeded from the saved
            inputs and the saved start-of-chunk states ``Hbound`` -- the
            verified, exact-to-eager reverse-chunk recurrence. A fused Triton
            backward trio was built and validated but measured 8-12x slower
            than ``torch.compile`` on an L4 and reverted (see this class's and
            the module's docstrings); there is no dispatcher and no fallback
            branch here now. Returns grads positionally for
...
def _run_delta_grad_parity_body(collect: list[dict] | None = None) -> int:
    ...
    Both ``check_raw`` and ``check_module`` validate the (kernel-forward +
    torch-composed backward) path numerically against the eager reference,
    with the fp32-vs-fp64 own-deviation gate. The backward is
    ``_delta_backward_torch`` called directly by ``_DeltaScanFn`` -- the fused
    Triton backward was measured 8-12x slower than compile on L4 and reverted
    (module docstring), so there is no fused-kernel engagement to assert here
    any more; grad parity is numeric-only.
    """
    from mingru import min_gru
    ...
    def check_raw(tag: str, d_k: int, nh: int, chunk_size: int, T: int) -> None:
        ...
        # Kernel-forward + torch-composed backward (`_delta_backward_torch`,
        # via `_DeltaScanFn`); grad parity is numeric-only now that the fused
        # Triton backward is reverted (module docstring).
        loss_t.backward()
    ...
    def check_module(name: str, mixer_fn) -> None:
        ...
        try:
            ...
            return
        # Kernel-forward + torch-composed backward; grad parity is numeric-only
        # (the fused Triton backward is reverted -- module docstring).
        (cot * out_t).sum().backward()
        grads_t = {nm: p.grad.detach().clone() for nm, p in mixer.named_parameters()}
        os.environ["MINGRU_SCAN"] = "eager"
