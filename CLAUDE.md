...
- Gated-delta decay round (design review 2026-07-21, deferred by owner decision; PASS verdict, no S3+, suite green, intent-reviewer PASS). Two latent observations, address only if they grow: (1) `DeltaMinGRU.forward` now has two dispatch shapes selected by decay state: the `decay=None` path uses the `_delta_scan_should_try` + `_dispatch_delta_scan` kernel pair (region gate, silent eager for out-of-region CUDA-`auto`), while the decay-active path uses `_delta_decay_dispatch_guard` (no region gate, warn-once eager for CUDA-`auto`). Correct tradeoff today (an eager-only decay path genuinely cannot share the kernel envelope logic); collapse into one dispatch mechanism if/when a Triton decay kernel lands (retiring the two-shape split). Building the unified shape now would be speculative. (2) `_warned_delta_decay_fallback` is the fourth parallel module-global warn-once flag (`_warned_scan_fallback`/`_warned_angle_fallback`/`_warned_delta_fallback`); the established convention with a distinct meaning each, but watch for a hoist if a fifth dispatch seam appears.
