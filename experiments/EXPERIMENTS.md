...
**Addendum (2026-07-25, same day).** Independent cross-check of the user-downloaded shard job logs against the ledger: all 324 `bench-churn-02` cells present in both, zero missing/extra. Shard 0 independently re-ran the 9 pilot-overlap cells (log/gru/signed × seeds 0–2; dedup discarded the re-runs); comparing the discarded re-run payloads to the retained pilot rows, every metric (`ckpt.val_auc`, `acc.test`) is bit-identical across all 9 cells — only wall-clock `secs` differs. The pilot-phase rows are therefore empirically, not just configurationally, indistinguishable from matrix rows: a free same-stratum reproducibility check for the round.
