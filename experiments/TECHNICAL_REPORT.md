...
## 8. Why the investigation required multiple cycles
...
`GivensMinGRU`'s parallel $k\times k$ scan is not the cheap path on CPU: one uncontended forward+backward step at $B=128$, $T=64$ costs $0.961$s against $0.179$s for the sequential delta-rule path, and no parallel-scan configuration measured here beats the sequential rank-$1$ delta implementation on CPU. Its promotion rests on keeping the composer inside this repository's parallel associative scan together with fit reliability and length generalization within the rotation family, not on training-step speed. Scope disclosure: the delta rule itself is parallelizable over sequence length — the DeltaNet literature's chunked WY representation is a parallel algorithm with released tuned kernels — but that form is not implemented here, so the sequential-vs-parallel contrast above is between this repository's implementations, not between the mechanisms, and no comparison against tuned parallel delta kernels has been measured.
...
- **Extract-then-compose stacks that must derive an operation from raw input and then compose it** ($S3\text{-hier}$-like), when the composer should run inside this library's parallel associative scan: `GivensMinGRU` as the composer (`mixer="givens"`, `block_size=8`, `rounds=3`) above a signed extractor. Evidentiary basis: $8/12$ fit rate at matched $64$-element state, $+2.2\%$ parameters, leading the reliably-trainable field at $0.927/0.733$ fit-only at $T=512/1024$. Main risks: continuity gives no attractor, so accuracy still decays by $T=1024$ and no continuous composer reaches the snapped composer's rare exact $0.983$; the parallel $k\times k$ scan is slower than this repository's sequential delta path on CPU; and the "parallel" justification is scoped to this repository — parallel delta forms (chunked WY) exist in the literature but are not implemented or benchmarked here.
