...
`DeltaMinGRU`'s chunked-WY `forward` is not one of the four scan ops. It does have its own hand-written Triton kernel (a forward trio plus a torch-composed backward, gated separately from the four scan ops), but `torch.compile` — not this kernel — is the recommended CUDA path for `mixer="delta"`: a CUDA fusion-headroom probe found `torch.compile` recovers 68–89% of the available fusion headroom, while the kernel runs 1.85x–3.79x slower than compile on every probed shape. Under `MINGRU_SCAN=auto` the kernel therefore engages only in a narrow measured win region (long sequences, narrow head dims) where it beats eager and saves ~3x memory, and stays eager everywhere else. See the [Givens & Delta deep dive](givens-delta.md#costs-and-the-two-axis-recommendation) for the measurement and the GPU cost comparison between the two composers.
