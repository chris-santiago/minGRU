...
- **Time decay (eager-only).** `DeltaMinGRU` accepts the same `decay=`/`decay_rate=`/`log1p_delta=` keywords as the other mixers: a Gated-DeltaNet-style per-token per-head scalar gate that preserves the chunked-WY parallel `forward` (`decay=None` stays bit-identical). With decay active the forward is eager-only, so `MINGRU_SCAN=triton` fails loud and `torch.compile` is the recommended CUDA path. Under float32, set `log1p_delta=True` for very large or sentinel (`+inf`) gaps: the chunked forward carries a per-token cumulative log-decay, and an uncompressed gap near the `1e10` `+inf`-sanitizer cap loses float32 precision for tokens sharing a chunk with it (output stays finite but not chunk-size invariant). See the README's "Time-aware decay" note.
