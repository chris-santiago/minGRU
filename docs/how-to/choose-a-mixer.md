...
- **Time decay (eager-only).** `DeltaMinGRU` accepts the same `decay=`/`decay_rate=`/`log1p_delta=` keywords as the other mixers: a Gated-DeltaNet-style per-token per-head scalar gate that preserves the chunked-WY parallel `forward` (`decay=None` stays bit-identical). With decay active the forward is eager-only, so `MINGRU_SCAN=triton` fails loud and `torch.compile` is the recommended CUDA path.
