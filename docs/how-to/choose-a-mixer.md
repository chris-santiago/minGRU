...
- **Time decay (eager-only).** `DeltaMinGRU` accepts the same `decay=`/`decay_rate=`/`log1p_delta=` keywords as the other mixers: a Gated-DeltaNet-style per-token per-head scalar gate that preserves the chunked-WY parallel `forward` (`decay=None` stays bit-identical). With decay active the forward is eager-only, so `MINGRU_SCAN=triton` fails loud and `torch.compile` is the recommended CUDA path. Under float32, very large or sentinel (`+inf`) gaps are handled correctly automatically: the chunked forward carries a per-token cumulative log-decay, and an internal float32 clamp on it keeps output finite, chunk-size invariant, and oracle-matching even when a gap saturates (the affected token's memory is fully wiped). The module warns once when this happens. `log1p_delta=True` is then recommended (not required for correctness) if you want very large gaps to stay *distinguishable* rather than all saturating to a full wipe. See the README's "Time-aware decay" note.
