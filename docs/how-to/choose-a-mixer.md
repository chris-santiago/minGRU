...
- **Time decay (eager-only).** `DeltaMinGRU` accepts the same `decay=`/`decay_rate=`/`log1p_delta=` keywords as the other mixers: decay is supported, eager-only on delta. See [Enable time decay](enable-time-decay.md#delta-decay-eager-only) for the mechanism, the `delta_t` contract, and the float32 large-gap behavior.
